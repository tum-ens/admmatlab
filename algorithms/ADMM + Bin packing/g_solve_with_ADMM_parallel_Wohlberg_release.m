function [cpx,central_soln] = g_solve_with_ADMM_parallel_Wohlberg_release(select, A, lhs, rhs, ub, lb, ...
...
%Verschiedene Indizmengen definieren fï¿½r jeweilige Cluster
global_var_set = [];
global_int_set = [];
...
for iter=1:maxiter
    ...
    if iter>1
        ...
        for cluster_idx=1:numel(cluster_names)
            ...
            resid_dual_rel{cluster_idx}=rhos{cluster_idx}(1)*norm(x_diff{cluster_idx})/norm([l_collected{:}]);
