function [cpx, central_soln] = g_solve_with_ADMM_parallel_Song_local_rho_release(select, A, lhs, rhs, ub, lb, ...
...
for iter=1:maxiter
    ...
    %Relevante globale Werte fï¿½r alle Cluster zuweisen
    for cluster_idx=1:numel(cluster_names)
    ...
    for cluster_idx=1:numel(cluster_names)
        ...
        %lineare Kostenterme addiert auf die ursprï¿½ngliche Kostenfunktion
        ...
        sub_models{cluster_idx}.Model.Q=spdiags(diag_aux{cluster_idx},0,...
            size(internal_plus_neighbor_var_indices{cluster_idx},1),size(internal_plus_neighbor_var_indices{cluster_idx},1));
    ...
    for cluster_idx=1:numel(cluster_names)
        ...
        %Differenz zwischen Iterationen berechnet fï¿½r Dualresidual
        if iter>1
            x_diff{cluster_idx} = ...
    ...
    if iter>1
        for cluster_idx=1:numel(cluster_names)
            ...
            % cluster specific penalty parameters (no convergence guarantee
            % but makes faster)
    ...
    if iter > 1
        ...
        elseif convergence_type == 2
            if abs(central_soln_obj - f*Plink_global)/central_soln_obj < convergence_tolerance
                break
            end
        end
    end
